...
OmniStudio 是一个面向团队的自托管图片与视频生成工作台：文生图、整图编辑、局部重绘、文生视频、图生视频、首尾帧视频，内置素材库、额度管理、用户组、工作团队与模型权限控制。

## ✨ 功能

- **生成**：文生图、参考图编辑、蒙版局部重绘、文生视频、图生视频、首尾帧
...
- **工作台**：图片/视频模式切换，历史记录、重新生成、重试、下载、参考图、首尾帧与遮罩绘制
- **素材**：会话、素材库（可按类型、来源、模型、日期、备注/提示词筛选）、工作团队分享参考图、缩略图、存储配额
- **管理**：用户审批、用户组（模型权限）、工作团队（素材分享）、组级生成额度（图片按张、视频按秒）
